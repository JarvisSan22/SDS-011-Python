...
def readrpi3data():
    DataFolder = "/home/pi/SDS-011-Python/AQ/Data/"
    ...
    for file in todayfiles:
        ...
        data = pd.concat([data, dataloop],
                         ignore_index=False, axis=0, sort=True)
...
def sensorsmean(data, sensors):
    ...
    if "DHT22" in Meandata.keys():
        ...
        dhtdata = data[["DHT-RH", "DHT-T"]]
        dhtdata.rename(columns={"DHT-RH": "RH", "DHT-T": "Temp"}, inplace=True)
    return sdsdata, dhtdata


def runsen():
    os.system("sudo python /home/pi/SDS-011-Python/AQ/Scripts/start.py &")
    return "yes"
...
def newdata(deviceName, action):
    ...
    if deviceName == "kill":
        os.system(
            "sudo pkill -f /home/pi/SDS-011-Python-master/AQ/Scripts/start.py ")
        data, sens = readrpi3data()
        sdsdata, dhtdata = sensorsmean(data, sens)
