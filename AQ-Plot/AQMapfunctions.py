...
def Walkdata(loc):
    try:
        ...
        df.set_index('time', inplace=True, w=True)
...
def plotdataPopInfo(df,vals,info,locname):
    ...
    <head>
      ...
      <div class="row">
    	<div class="column">
        <iframe src= '"""+figname+"""' name="sample" width="300" height="360">
        </iframe>
    	</div>
        <br />
        <div class="column">
...
def genmap(Datadic,val,titlename,infos,locname):
    ...
    for k, data in Datadic.items():
       ...
       # getting sensors name 
        ...
        DataMarkerInfo(df,val,startlat,startlon,walkg,info,locname)
        ...
        try:
            ...
            if Stilldic!="ERROR": #plot data as a circle 
                for sk,stdf in Stilldic.items():
                    slat=stdf["lat"].mean()
                    slon=stdf["lon"].mean
                    DataCircle(stdf,slat, slon,walkg,val)
            else:
                print("--No still data--")
